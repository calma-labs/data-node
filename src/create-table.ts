...
import type { BQSchemaField } from './types.js';

const PROJECT = 'calmal';
const DATASET = 'lending_poc';
const SA: string = process.env.IMPERSONATE_SA || 'lending-poc@calmal.iam.gserviceaccount.com';

const POOL_SCHEMA: BQSchemaField[] = [
...
const SNAPSHOTS_SCHEMA: BQSchemaField[] = [
  { name: 'snapshotId', type: 'STRING', mode: 'REQUIRED' },
...
(async () => {
  ...
  for (const name of ['pool', 'snapshots'] as const) {
